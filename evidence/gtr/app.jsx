...
function FieldworkSlidePage() {
  return (
    <section className="report-section" id="fieldwork-slides">
      <ChapterLabel number="1.2b">First Prototype / Fieldwork slides</ChapterLabel>
      <div className="report-document">
        <PageIntro
          eyebrow="Presentation form"
          title="Fieldwork week — slide narrative"
          summary="The deck form of the five founder-role sessions. Use this for partner reviews; the full method, figures, and aggregated findings live in the fieldwork report."
          links={[
            ["/gtr/docs/fieldwork-report/", "Full fieldwork report"],
            ["/gtr/docs/fieldwork-report/feedback/", "Presentation feedback"],
            ["/gtr/docs/stage-1/", "Stage 1 PRD"],
          ]}
        />
        <section className="report-chapter">
          ...
          <p className="report-lead">Five founder-role walkthroughs. Ted was the only actual founder; the others brought technical, investment, peer-design, and accessibility lenses.</p>
          <p>This round identifies what to retest. It does not validate calculation accuracy or market demand.</p>
        </section>
        <section className="report-chapter">
          <span className="report-number">1</span>
          <h2>Agenda</h2>
          <div className="report-table-scroll">
            <table className="report-table">
              <thead><tr><th>Section</th><th>What it decides</th></tr></thead>
              <tbody>
                <tr><td>Storyboard → prototypes</td><td>What we built and which A/B pairs matter</td></tr>
                <tr><td>Testing process</td><td>Who we tested, what failed, what worked</td></tr>
                <tr><td>What we learned</td><td>How to read directional evidence</td></tr>
                <tr><td>Next direction</td><td>Challenges, pivots, and the hybrid prototype</td></tr>
              </tbody>
            </table>
          </div>
        </section>
        <section className="report-chapter">
          <span className="report-number">2</span>
          <h2>Findings that drive product change</h2>
          <div className="report-table-scroll">
            <table className="report-table report-table-wide">
              <thead><tr><th>Finding</th><th>Implication</th></tr></thead>
              <tbody>
                {aggregatedFindings.map(([finding, , implication]) => (
                  <tr key={finding}><td>{finding}</td><td>{implication}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
...
function FieldworkFeedbackPage() {
  return (
    <section className="report-section" id="fieldwork-feedback">
      ...
      <div className="report-document">
        <PageIntro
          eyebrow="Critique notes"
          title="What the fieldwork presentation still has to prove"
          summary="Feedback on the fieldwork deck and report: clarity of audience, strength of evidence claims, and what to keep separate from product-market validation."
          links={[
            ["/gtr/docs/fieldwork-report/", "Fieldwork report"],
        ...
        <section className="report-chapter">
          <span className="report-number">0</span>
          <h2>Keep the claim sized correctly</h2>
          <p className="report-lead">Directional prototype evidence is not product-market fit.</p>
          <p>Reviewers and facilitators should frame the five sessions as comprehension and structure tests — not as proof that founders will pay, that the carbon math is certified, or that the funnel converts in the wild.</p>
          <aside className="report-note"><b>Language to prefer</b><p>“What to retest next” over “validated.” “Founder-role participants” over “five founders” unless naming Ted specifically.</p></aside>
        </section>
        <section className="report-chapter">
          <span className="report-number">1</span>
          <h2>Session observations that still need product response</h2>
          <div className="report-table-scroll">
            <table className="report-table report-table-wide">
              <thead><tr><th>Participant</th><th>Area</th><th>Observation</th></tr></thead>
              <tbody>
                {sessionObservations.map(([who, area, obs], i) => (
                  <tr key={`${who}-${area}-${i}`}><td>{who}</td><td>{area}</td><td>{obs}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
        ...
        <section className="report-chapter">
          <span className="report-number">2</span>
          <h2>Where the archive goes next</h2>
          <ul>
            <li>Stage 1 PRD freezes the MVP boundary that fieldwork did not invent — it only pressure-tested it.</li>
            <li>Stage 2 PRD captures the funnel, privacy, and evidence-library work that sessions forced into the open.</li>
...
function ProductDeckPage() {
  return (
    <section className="report-section" id="product-deck">
      <ChapterLabel number="D">Product deck</ChapterLabel>
      <div className="report-document">
        <PageIntro
          eyebrow="Climate Goal Platform"
          title="From service story to product story"
          summary="The product deck converts the earlier advisory framing into a self-serve +/− impact dashboard narrative. The research report is the long form of the same argument."
          links={[
            ["/gtr/", "Research report"],
            ["/gtr/docs/stage-1/", "Stage 1 PRD"],
          ]}
        />
        <section className="report-chapter">
          <h2>Deck takeaways</h2>
          {reportTakeaways.map((item) => (
            <div key={item.title} className="report-note" style={{ marginBottom: "1rem" }}>
              <b>{item.title}</b>
              <p>{item.body}</p>
            </div>
          ))}
        ...
        <section className="report-chapter">
          <h2>Competitive landscape (from the report)</h2>
          <div className="report-table-scroll">
            <table className="report-table">
              <thead><tr><th>Category</th><th>Offer</th><th>Gap</th></tr></thead>
              <tbody>
                {competitiveLandscape.map((row) => (
                  <tr key={row.category}><td>{row.category}</td><td>{row.offer}</td><td>{row.gap}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
...
function PartnersDeckPage() {
  return (
    <section className="report-section" id="partners-deck">
      <ChapterLabel number="P">GTR Partners</ChapterLabel>
      <div className="report-document">
        <PageIntro
          eyebrow="Earlier framing"
          title="Where the project started"
          summary="The partners deck captures the advisory and transition-service model: measure, strategize, support, workshops. That model is valuable — and does not scale past one-to-one engagements. The product wedge is the repeatable core."
          links={[
            ["/gtr/", "How the direction rescoped"],
            ["/gtr/docs/stage-1/", "Stage 1 PRD"],
          ]}
        />
        <section className="report-chapter">
          ...
          <div className="report-table-scroll">
            <table className="report-table">
              <thead><tr><th>Before</th><th>Now</th><th>Implication</th></tr></thead>
              <tbody>
                <tr><td>One-to-one consulting and workshops</td><td>Self-serve gamified dashboard</td><td>Value has to survive without a human service layer.</td></tr>
                <tr><td>Broad startup climate awareness</td><td>Climate startups first</td><td>Start with the buyer who already underwrites impact.</td></tr>
                <tr><td>Climate Brick as the model</td><td>Climate Brick as a reference shelf</td><td>The product needs its own method.</td></tr>
                <tr><td>Impact as adjacent context</td><td>Impact as operating surface</td><td>Weekly, owned, and credible.</td></tr>
              </tbody>
            </table>
          </div>
        </section>
...
function App() {
  ...
  if (path.includes("/docs/fieldwork-report/slides") || path.includes("/slides/fieldwork-week")) {
    ...
    docsPage = "stage-2";
  } else if (path.includes("/slides/climate-goal-platform")) {
    docsPage = "product-deck";
  } else if (path.includes("/slides/gtr-partners")) {
    docsPage = "partners-deck";
  }
  ...
  return (
    <div id="top">
      ...
      <main>
        ...
        {activeChapter === "first-prototype" && docsPage === "product-deck" && <ProductDeckPage />}
        {activeChapter === "first-prototype" && docsPage === "partners-deck" && <PartnersDeckPage />}
